...
function renderDevices() {
    if (!devices.length) {
        deviceTableBody.innerHTML =
            '<tr><td colspan="7">ยังไม่มีข้อมูลอุปกรณ์</td></tr>';
...
deviceTableBody.addEventListener("click", event => {
    ...
    const editButton =
        event.target.closest(".edit-device-button");
    ...
    const manageButton =
        event.target.closest(".manage-device-button");

    if (manageButton) {
        openManageDevice(manageButton.dataset.id);
    }
});
...
function createOrderSelector() {
    ...
    let wrapper =
        document.getElementById("manageOrderSelectorWrapper");
    ...
    if (!wrapper) {
        ...
        wrapper.id =
            "manageOrderSelectorWrapper";

        wrapper.style.marginBottom = "15px";

        wrapper.innerHTML = `
            <label
                for="manageOrderSelect"
            ...
            >
                Accessories และรูปภาพจะถูกผูกกับ Order ที่เลือก
            </small>
        `;

        manageModal.insertBefore(
            wrapper,
            manageModal.firstChild
        );
    }
...
async function loadOrdersForDevice(deviceId) {

    managingOrders = [];
    managingOrderId = null;

    const wrapper = createOrderSelector();
    const select =
        document.getElementById("manageOrderSelect");

    select.innerHTML =
        '<option value="">กำลังโหลด Order...</option>';
    ...
    if (error) {
        ...
        console.error(
            "โหลด Orders ของ Device ไม่สำเร็จ:",
            error
        );

        select.innerHTML =
            '<option value="">โหลด Order ไม่สำเร็จ</option>';
    ...
    if (!managingOrders.length) {

        select.innerHTML =
            '<option value="">-- ไม่มี Order --</option>';

        accessoryForm.style.display =
            isStaff ? "none" : "none";
    ...
    managingOrders.forEach(order => {
        ...
        option.textContent =
            `${order.order_code} | ${getOrderStatusText(order.status)} | ${Number(order.repair_cost || 0).toFixed(2)} บาท`;

        select.appendChild(option);
    });

    // ถ้ามี Order เดียว ให้เลือกให้อัตโนมัติ
    if (managingOrders.length === 1) {
...
function updateManageOrderUI() {
    ...
    if (!managingOrderId) {
        ...
        accessoryList.innerHTML =
            '<p class="empty-text">กรุณาเลือก Order ก่อน</p>';
...
async function openManageDevice(deviceId) {
    ...
    manageSubtitle.textContent =
        `${device.device_type || "-"} ${device.brand || ""} ${device.model || ""} | S/N: ${device.serial_number || "-"}`;
    ...
    await loadOrdersForDevice(deviceId);
...
async function loadAccessories() {
    ...
    if (error) {
        ...
        accessoryList.innerHTML =
            '<p class="empty-text">โหลด Accessories ไม่สำเร็จ</p>';
    ...
    if (!data?.length) {
        ...
        accessoryList.innerHTML =
            '<p class="empty-text">ยังไม่มี Accessories</p>';
        ...
        data.map(item => `
            ...
            <div class="item-row">
                ...
                <span class="item-name">
                    ${escapeHtml(item.accessory_name)}
                </span>
                ...
                ${
                    isManager
                        ? `
                            <button
                                class="danger-button delete-accessory-button"
                                data-id="${item.id}"
                            >
                                ลบ
                            </button>
...
accessoryForm.addEventListener(
    ...
    async event => {
        ...
        if (
            ...
            accessoryMessage.textContent =
                "กรุณาเลือก Order ก่อน";

            return;
        }

        const name =
            accessoryName.value.trim();
...
accessoryList.addEventListener(
    "click",
    async event => {
        ...
        if (
            !confirm(
                "ต้องการลบ Accessories รายการนี้หรือไม่?"
            )
        ) {
            return;
        }
...
async function loadImages() {
    ...
    if (error) {
        ...
        imageList.innerHTML =
            '<p class="empty-text">โหลดรูปภาพไม่สำเร็จ</p>';
    ...
    if (!data?.length) {
        ...
        imageList.innerHTML =
            '<p class="empty-text">ยังไม่มีรูปภาพ</p>';
                ...
                <div class="image-card-footer">
                    ...
                    ${
                        isManager
                            ? `
                                <button
                                    class="danger-button delete-image-button"
                                    data-id="${image.id}"
                                    data-path="${encodeURIComponent(image.image_url)}"
                                >
                                    ลบ
                                </button>
                            `
    ...
    imageList.innerHTML =
        cards.length
            ? cards.join("")
            : '<p class="empty-text">ไม่สามารถแสดงรูปภาพได้</p>';
}
...
uploadImagesButton.addEventListener(
    ...
    async () => {
        ...
        if (invalid) {
            ...
            imageMessage.textContent =
                `ไฟล์ ${invalid.name} ไม่รองรับ หรือมีขนาดเกิน 10 MB`;
        ...
        for (const file of files) {
            ...
            const storagePath =
                `${managingDeviceId}/${managingOrderId}/${uniqueName}`;
            ...
            if (insertError) {
                ...
                console.error(
                    "บันทึก device_images ไม่สำเร็จ:",
                    insertError
                );

                /*
                 * ถ้าบันทึก DB ไม่สำเร็จ
                 * พยายามลบไฟล์ที่เพิ่ง Upload
                 */
        ...
        imageMessage.textContent =
            `อัปโหลดสำเร็จ ${successCount}/${files.length} รูป`;
...
imageList.addEventListener(
    ...
    async event => {
        ...
        if (dbError) {
            ...
            console.error(
                "ลบข้อมูล device_images ไม่สำเร็จ:",
                dbError
            );
